...
export default function UserHistoryPage() {
  ...
  const [statusFilter, setStatusFilter] = useState<'ALL' | 'DIKEMBALIKAN' | 'REJECTED'>('ALL');
  ...
  const exportHistory = () => {
    try {
      // Simple CSV export
      const headers = ['Nama Barang', 'Kode Barang', 'Kategori', 'Merek', 'Lokasi', 'Status', 'Tanggal Pengajuan', 'Tanggal Selesai'];
  ...
  const returnedCount = history.filter(item => item.status === 'DIKEMBALIKAN').length;
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white rounded-lg shadow p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-sm font-medium text-gray-600">Total Riwayat</p>
            ...
            <div className="border-t pt-4 mt-4">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  ...
                  <select
                    value={statusFilter}
                    onChange={(e) => setStatusFilter(e.target.value as 'ALL' | 'DIKEMBALIKAN' | 'REJECTED')}
                  ...
                  return (
                    ...
                    >
                      ...
                      {(item.fotoPinjam || item.fotoKembali) && (
                        <div className="bg-white rounded-lg p-4 mb-4">
                          <h4 className="font-medium text-gray-900 mb-4">Dokumentasi</h4>
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {item.fotoPinjam && (
                              <div>
                                <h5 className="text-sm font-medium text-gray-700 mb-2">Foto Saat Dipinjam</h5>
                                <div className="aspect-w-16 aspect-h-12 bg-gray-100 rounded-lg overflow-hidden">
                                  <img
                                    ...
                                    onClick={() => item.fotoPinjam && window.open(item.fotoPinjam, '_blank')}
                                  />
                                </div>
                            ...
                            {item.fotoKembali && (
                              <div>
                                <h5 className="text-sm font-medium text-gray-700 mb-2">Foto Saat Dikembalikan</h5>
                                <div className="aspect-w-16 aspect-h-12 bg-gray-100 rounded-lg overflow-hidden">
                                  <img
                                    src={item.fotoKembali}
                                    alt="Foto saat dikembalikan"
                                    className="w-full h-48 object-cover cursor-pointer hover:opacity-75 transition-opacity"
                                    onClick={() => item.fotoKembali && window.open(item.fotoKembali, '_blank')}
                                  />
                                </div>
                                <p className="text-xs text-gray-500 mt-1">Klik untuk memperbesar</p>
                              </div>
                      ...
                      {(item.penanggungJawab || item.approvedByUser) && (
                        <div className="mt-4 pt-4 border-t border-gray-200">
                          ...
                          {item.approvedByUser && (
                            <p className="text-sm text-gray-600">
                              <span className="font-medium">Diproses oleh:</span> {item.approvedByUser.nama}
                            </p>
                          )}
                        </div>
                      )}
